...
def build_remote_mcp(
    ...
    returned in diagnostics or CLI output. Every tool must be explicitly
    ...
    if not names:
        raise MCPConfigurationError("At least one MCP tool must be explicitly allowlisted")
    server = {"type": "url", "name": server_name, "url": url}
    if token_env:
        token = os.getenv(token_env)
        if not token:
            raise MCPConfigurationError(f"MCP token environment variable '{token_env}' is not set")
        server["authorization_token"] = token
    toolset = {
        "type": "mcp_toolset",
        "mcp_server_name": server_name,
        "default_config": {"enabled": False, "defer_loading": True},
        "configs": {name: {"enabled": True, "defer_loading": True} for name in names},
    }
